...
- `docs/features/chat-interface.md` — Chat panel: send, streaming markdown, tool call cards, phase display, meta-driven model selector, CSP, theme adaptation
- `docs/features/agent-operations.md` — File read/write/edit and bash execution (wired to orchestrator pipeline)
- `docs/features/configuration.md` — VS Code settings: provider selector, single baseUrl/apiKey, coordinator/worker models, SecretStorage, model switching
- `docs/architecture/core-package.md` — Core engine: agent factory (single-provider model registry), structured output, model abstraction, config layering, worktree/diff/planner/orchestrator
- `docs/architecture/extension-package.md` — Extension: activation, message protocol (8 event types), webview architecture, meta-injected model list, core integration
