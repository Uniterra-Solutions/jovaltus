...
來源：`packages/core/src/model/errors.ts:1-50`、`packages/core/src/model/anthropic-provider.ts`、`packages/core/src/model/openai-provider.ts`
...
`parseRetryAfter()`（`model/errors.ts:32-36`）從 HTTP `retry-after` 標頭解析建議等待時間。
...
每個提供者（OpenAIProvider、AnthropicProvider）透過相同的三層分類邏輯處理 SDK 例外（`model/openai-provider.ts`、`model/anthropic-provider.ts`）：
...
- **Edit Tool**：比對失敗時回傳 `{ filePath, error: 'not_found' | 'ambiguous' }`（`agent/tools/edit.ts`）
- **Bash Tool**：執行失敗時回傳完整的 stdout/stderr 與 exit code，不拋出例外（`agent/tools/bash.ts`）
